...
RETIRED = [
    ...
    # --- 第 1 版（Zenodo 10.5281/zenodo.22505179）に残っていた対照 20 個の頃の値 ---
    # 2026-09-23、教科書の執筆で CSV から再計算して見つかった。正しい値の側が照合に
    # 入っていなかったので、audit は 517/517 一致のまま通っていた。第 2 版の
    # 「Changes from version 1」は旧値を別の語順で書き、ここに当たらないようにしてある。
    ("the 0.048 of size", "NOX2 の対照平均。対照 20 個の頃の値（正: 0.057）"),
    ("ランダム対照（0.048）", "同上（和文）"),
    ("expected 0.708", "100 遺伝子以上の α の期待値。同上（正: 0.710）"),
    ("期待値は 0.708", "同上（和文）"),
    ("157 (47.1%)", "α>0.7 で 2 対照を通らないセット。同上（正: 234 (70.3%)）"),
    ("157 セット（47.1%）", "同上（和文）"),
    ("a difference of 0.110", "中央値どうしの差（0.108）と読める。0.110 はセットごとの超過の中央値"),
    ("との差は 0.110", "同上（和文）"),
...
def checks() -> list[tuple[str, float, int, bool]]:
    ...
    if len(nox2) == 1:
        ...
        # 同じ文で「これを下回る」と比べている対照の値。第 1 版には対照 20 個の頃の
        # 0.048 が残っていた。ここに無かったので audit を素通りしていた（2026-09-23）。
        add("NOX2 アンカーの対照（発現量そろえ）の平均", r.null_mean, 3)

    # --- 補遺 S7: α はサイズで上がる ---
    # 第 1 版では 3 値（期待 α 0.708、対照を超えない 157 セット・47.1%）が対照 20 個の
    # 頃のまま残っていた。照合の対象に入っていなかったため audit を素通りしていた。
    # 「対照を超えない」は本文全体の合否と同じく 2 種の対照の両方で数える。
    _big = m[m.n_genes_tested >= 100]
    add("[S7] 100 遺伝子以上のセットの α 中央値", _big.cronbach_alpha.median(), 3)
    add("[S7] 同・対照から計算した α の期待値の中央値", _big.alpha_null_expected.median(), 3)
    _hi = m.cronbach_alpha.gt(0.7)
    add("[S7] α が 0.7 を超えるセット数", int(_hi.sum()), 0)
    add("[S7] うち 2 対照を通らないセット数", int((_hi & ~both).sum()), 0)
    add("[S7] うち 2 対照を通らない割合(%)", 100 * (_hi & ~both).sum() / _hi.sum(), 1)
    ...
    # 代理変数と遺伝子を共有していれば、相関の一部は同じ遺伝子を両側に入れた帰結である。
    # 双方から共有遺伝子を除いた版を登録する（原稿の主張はこちらに合わせる）。
    add("[通過17件] マーカーと共有する遺伝子数 中央値", int(_pc["マーカーと共有する遺伝子数"].median()), 0)
    add("[通過17件] 重複除去後に評価できた件数", int(_pc["重複除去後のq"].notna().sum()), 0)
    add("[通過17件] 重複除去後に対照を上回る件数", int((_pc["重複除去後のq"] < 0.05).sum()), 0)
    add("[通過17件] 重複除去後の超過 中央値", float(_pc["重複除去後の超過"].median()), 3)
