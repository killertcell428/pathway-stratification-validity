...
PATTERNS: list[tuple[str, str, str]] = [
    ...
    ("クライアント",
     ...
     "案件由来であることを示す語"),
    # 2026-09-02 に方針転換。所属を `Cytra Inc., Tokyo, Japan` として出すことに決めたため、
    # 会社名そのものの検出ルールは撤去した（奨励研究の在籍証明と整合を取る方向の変更）。
    # 雇用関係を語る文言（下の規則）とクライアント案件の規則は残してあるので、
    # 「シトラの案件で」のような書き方は依然として止まる。
    ("勤務先",
    ...
    ("個人情報",
     ...
     "研究と無関係な個人情報"),
    ("個人情報",
     r"[A-Za-z0-9._%+-]+@(?:gmail|yahoo|outlook|hotmail|icloud|me)\.(?:com|co\.jp|ne\.jp|jp)",
     "個人のフリーメールアドレス。連絡先は所属メールで出すので公開物には要らない"),
    ("ローカルパス",
