...
なぜこのモジュールが要るか
  ...
  シグネチャの z は同じ枠の対照全部を基準にしているので大きく外れうる（実測で z = 12 に達する）
  ...
  そこで帰無を**交換可能性検定**に組み替えた。各枠にはシグネチャ 1 個と対照 N 個、
  合わせて N+1 個の遺伝子集合がある（N は配列長から決まる。事前登録では N = 10,000）。帰無仮説「そのシグネチャはサイズと発現量をそろえた
  ランダム集合と交換可能である」の下では、どれを「シグネチャ」と呼ぶかは任意である。
  そこで各枠から N+1 個のうち 1 個を選び、**残り N 個を基準に標準化する**。
...
def excess_z(values: np.ndarray, index: int) -> float:
    ...
    観測側（シグネチャを対照 N 個で標準化）と帰無側（対照 1 個を残り N 個で標準化）で
...
def slot_statistic(coherence: list[np.ndarray], target: list[np.ndarray],
    ...
    coherence[i] / target[i] は枠 i の N+1 個（シグネチャ + 対照 N）の整合性と目的変数。
...
def exchangeability_null(coherence: list[np.ndarray], target: list[np.ndarray],
    ...
    各枠から N+1 個のうち 1 個を一様に選び、残り N 個を基準に標準化して
...
def range_matched_control_null(coherence: list[np.ndarray], target: list[np.ndarray],
    ...
    どうするか、そしてこれでは足りないこと
      ...
      **これは判定には使わない。記述的な診断にとどめる。** 合成データで確かめたところ、
      ...
      この残余は対照を増やしても解けないので、事前登録では限界として明記し、
